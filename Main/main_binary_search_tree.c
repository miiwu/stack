...
void main_binary_search_tree(void)
{
	...
	binary_search_tree_control_configuration_init(&tree_family, 4, sizeof(void *), NULL, NULL);
	...
	for (size_t cnt = 0; cnt < BINARY_SEARCH_TREE_DELETE_CASE; cnt++) {
		size_t count = cnt - '0';
		printf("\r\nb_tree.insert start: %c-%d \r\n", key_pool[cnt], count);
		tree_family_control_insert(tree_family, (void *)&key_pool[cnt]);
		printf("\r\nb_tree.insert end: %c-%d \r\n", cnt, count);
	}
	...
	printf("\r\n ------------------------+ binary search tree_family demo end +------------------------\r\n");
